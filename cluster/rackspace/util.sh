...
get-password() {
  ...
  if [[ -r "$file" ]]; then
    KUBE_USER=$(cat "$file" | python2.7 -c 'import json,sys;print json.load(sys.stdin)["User"]')
    KUBE_PASSWORD=$(cat "$file" | python2.7 -c 'import json,sys;print json.load(sys.stdin)["Password"]')
    return
  fi
  KUBE_USER=admin
